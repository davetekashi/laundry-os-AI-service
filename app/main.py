...
app = FastAPI(
    ...
    description=(
        ...
        "1. Call `POST /api/v1/context/prepare` when a laundry user logs in, using their authenticated `role`.\n"
        "2. Call `POST /api/v1/chat` using the same `laundry_id` and authenticated `role`.\n"
        ...
        "Important notes:\n"
        "- `/api/v1/chat` does not build context on demand. Matching laundry-and-role context must already be prepared.\n"
