...
app = FastAPI(
    ...
    description=(
        ...
        "1. Call `POST /api/v1/context/prepare` when a laundry user logs in, using their authenticated `role` and either `laundry_id` or `business_id`.\n"
        "2. Call `POST /api/v1/chat` using either resolved id and the same authenticated `role`.\n"
